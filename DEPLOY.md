...
**אם אחרי Reload השינויים עדיין לא נראים** (תופעה שקרתה בפועל בפרויקט זה):
```bash
touch /var/www/<username>_pythonanywhere_com_wsgi.py
```
ואז Reload שוב בלשונית Web. זה מאלץ את השרת לטעון את כל המודולים מחדש.
...
| Reload לא מחיל שינויי קוד חדשים | תהליך WSGI ישן נשאר תקוע בזיכרון של השרת | `touch /var/www/<username>_pythonanywhere_com_wsgi.py` ואז Reload שוב בלשונית Web |